...
(function () {
    ...
    function updateDisplay(data) {
        ...
        if (els.healthAdvisoryText) els.healthAdvisoryText.textContent = adv.text;
        if (els.alertTags) els.alertTags.innerHTML = adv.tags.map(t => `<span class="alert-tag">${t}</span>`).join('');
        ...
        try {
            ...
            if (merged.length > 0) {
                els.searchDropdown.innerHTML = merged.slice(0, 12).map(r => `
                    <div class="search-result" data-lat="${r.lat}" data-lon="${r.lon}" data-name="${r.name}" data-region="${r.region}" data-tz="${r.tz}">
                        <i class="fas fa-map-marker-alt"></i>
                        <span class="result-name">${r.name}</span>
                        <span class="result-region">${r.region}</span>
                    </div>`).join('');
